...
class Attach1DTo3D(Layer):
    # custom layer to handle attaching 1D pt_char tensor to model
    def __init__(self,ratio=4,**kwargs):
        # ratio lets you control how dimensional the input is, it will
        # duplicate the 1D until the depth of volumne : nonvolume data
        # equals the ratio
        self.ratio = ratio
    ...
    def build(self, input_shape):
        ...
        self.required_repeats = math.ceil(
            input_shape[0][-1] / input_shape[1][-1] / self.ratio
            ) # ceil ensures always at least one (0 would delete the tensor)
        super(Attach1DTo3D, self).build(input_shape)

    def call(self, inputs):
        x_3d, x_1d = inputs
        x_1d = tf.repeat(x_1d,self.required_repeats,axis=-1)
        for l in self.req_shape:
            x_1d = tf.stack([x_1d] * l, axis=-2)
        output = tf.concat([x_3d,x_1d],axis=-1)
        return output

    def compute_output_shape(self, input_shape):
        output_shape = [size for size in input_shape[0][0:-1]]
